...
The overlapping-annotation update adds an optional, index-only display layer
for both accession and manual-coordinate queries. It selects genes by inclusive
overlap with the current displayed range, shows one representative transcript
per additional gene on either strand, and recomputes that set when retained
data are zoomed. The queried gene retains its selected transcript or all of its
isoforms, remains the sole source of accession exon summaries and CDS guides,
and is excluded from the additional-gene count. Toggling this layer does not
issue a Zenodo request or change exact TSV data.
Every transcript-ID label under both live plots opens its parent gene's
VectorBase record in a new tab; downloaded SVGs retain the same links.

The query-summary v1 update adds a separate, versioned analytical contract:
`agamcs-query-summary-v1`. Matching Python and browser-JavaScript implementations
summarize the complete query plus selected-transcript CDS, UTR, intron, individual
exon, and strand-aware flank scopes. Every non-empty scope reports total bases,
finite-Cs bases and mean, accessible bases and fraction, finite accessible-base
SNP-density bases and mean, and the longest consecutive inaccessible run.
Overlapping exon bounds are unioned for partitions, while individual exon rows
remain distinct and ordered 5′→3′. Disconnected segments do not bridge runs.
Non-coding and unpadded absent scopes remain explicit zero-base rows with `NA`
ratios; QC-failed bases remain unknown rather than becoming zero.

The per-scope 80% result is explicitly a coverage audit against the existing
ranking threshold, not a new partition rank. Existing ranking cards are now
grouped under “Representative-transcript gene rankings.” A selected alternative
isoform changes the query summary, while its gene ranking remains pinned to the
representative transcript. Manual-coordinate queries produce only a query-span
summary unless an exact transcript annotation is supplied to the shared API.
...
- Accession heatmaps use the same versioned transcript annotation as the signal
  ...
  heatmap convention.
- Accession summaries use `agamcs-query-summary-v1` for query, selected-transcript
  CDS, UTR, intron, individual exon, and strand-aware flank scopes. Each scope
  exposes the Cs and accessibility/SNP denominators, and manual-coordinate
  queries remain query-only without an exact annotation.
...
- Bare multi-isoform gene queries render every transcript as an aligned row
  beneath both browser plots. The representative transcript is bold and supplies
  the selected-transcript summary and CDS guides; exact alternative-transcript
  queries instead summarize that selected model without changing static rankings.
...
The query-summary candidate passes:

- 95 Python tests, including shared query-summary fixtures and existing
  150,000- and 200,000-base Python/JavaScript plot-model parity;
- 96 JavaScript contract, summary, interaction, worker, accession, search, plot,
  and exact-TSV tests;
...
Desktop Chrome smoke tests on the local release candidate covered CYP6M2,
padding on both strands, Orco, the TIM `AGAP008288-RB` alternative isoform,
non-coding aga-mir-275, all-QC-failed AGAP010449, and a partial-accessibility
manual interval. Plots, keyboard submission, and the exact TSV download action
remained functional; analytics stayed unloaded without consent, and the browser
